...
router.get('/', function(req, res, next) {
  ...
  async.parallel([
      function(callback){
        ...
        rest.get('http://search:8002/artists/search?artist=' + artist).on('complete', function(data) {
          console.log('Artist ID is ' + data["id"]);
          rest.get('http://charts:8001/charts/' + data["id"]).on('complete', function(data) {
      ...
      function(callback){
          ...
          rest.get('http://search:8002/tracks/search?title=' + title + '&artist=' + artist).on('complete', function(data) {
            console.log('Title ID is ' + data["id"]);

            rest.get('http://images:8003/album/' + data["id"] + '/image').on('complete', function(data) {
